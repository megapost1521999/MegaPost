...
WHEN 1 THEN
    ...
    IF COALESCE(item.discount, 0) > 0 OR COALESCE(item.extra_payment_discount, 0) > 0 THEN
        final_text := final_text || chr(10) || 
...
ELSE
final_text := v_clean_title || chr(10) || chr(10) || '<blockquote>🟢 السعر النهائي ⇚ <b>' || final_price || ' جنيه</b>' || CASE WHEN old_price_val > final_price THEN ' (بدلاً من <s>' || old_price_val || ' جنيه</s>)' ELSE '' END || '</blockquote>' || chr(10) || chr(10) || '🔗 لينك المنتج:' || chr(10) || item.affiliate_link;
END CASE;
END IF;
