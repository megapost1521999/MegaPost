...
serve(async (req) => {
  ...
  try {
    ...
    const fileName = `smart_clip_${asin}_${Date.now()}.png`;
    const { error: uploadError } = await supabase.storage.from('banners').upload(fileName, buffer, { contentType: 'image/png' });
