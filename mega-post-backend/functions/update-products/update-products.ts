...
📌 <b>الاسم:</b> ${payload.title}
🆔 <b>ASIN:</b> <code>${payload.asin}</code>

💰 <b>السعر:</b> ${Math.floor(payload.oldPrice)} ← <b>${Math.floor(payload.newPrice)} ج.م</b>
  ...
  for (const config of allConfigs) {
    ...
    for (const chunk of chunks) {
      try {
        ...
        chunk.forEach(p => currentBatchPrices.set(p.asin, 0));
        amzItems.forEach((item: any) => {
          ...
          if (price === undefined || price === null) {
            price = item?.OffersV2?.Listings?.[0]?.SavingBasis?.Money?.Amount ?? 0;
          }
        ...
        for (const p of chunk) {
          ...
          console.log(`[Check] Device: ${deviceId} | ASIN: ${p.asin} | Old: ${p.price} | New: ${newPrice}`);

          if (Math.floor(newPrice) !== Math.floor(p.price)) {
            console.log(`⚠️ CHANGE DETECTED: Updating ${p.asin}...`);
